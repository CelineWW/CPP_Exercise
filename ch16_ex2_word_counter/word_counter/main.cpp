...
//	}
    ...
    auto duplicate_count = count_if(word_count.begin(), word_count.end(), over_1);
    cout << duplicate_count << " DUPLICATE WORDS" << endl;
...
void display(pair<string, int> word_pair) {
    cout << word_pair.first << "=" << to_string(word_pair.second) << ' ';
}

bool over_1(pair<string, int> word_pair) {
    return word_pair.second > 1;
}
